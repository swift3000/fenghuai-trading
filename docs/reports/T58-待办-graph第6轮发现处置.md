...
| R6-3 | P3 | sync-data/import-data 无权限时返回 success:false+message 但无 code 字段（其余函数有 code），错误响应结构不统一 | 登记，建议后续统一补 code（非阻塞，前端仅展示 message 行为正常） | 已做（T63-4 2026-09-02：两函数错误/成功路径统一补 code，success 保留兼容） |
