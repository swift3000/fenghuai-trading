...
console.log('\n' + '='.repeat(80))
console.log('🚀 丰淮商贸小程序 - 全角色业务流程测试开始')
    ...
    Object.keys(ROLE_DEFINITIONS).forEach(role => {
      ...
      describe(`2.${Object.keys(ROLE_DEFINITIONS).indexOf(role) + 1} ${roleDef.name}创建订单`, () => {
        ...
        test(`${roleDef.name} - 创建正常订单`, () => {
          const order = {
            orderNo: `丰淮商贸-${new Date().toISOString().slice(0,10).replace(/-/g,'')}-0001`,
            customerId: 'cust-001',
            customerName: '测试餐厅 A',
            items: [
        ...
        test(`${roleDef.name} - 订单号生成规则`, () => {
          ...
          assert(expectedPrefix.startsWith('丰淮商贸-20'), '订单号前缀格式正确', role)
        })
    ...
    ['admin', 'orderer', 'sorter', 'warehouse'].forEach(role => {
      ...
      describe(`3.${['admin','orderer','sorter','warehouse'].indexOf(role) + 1} ${roleDef.name}分拣操作`, () => {
        ...
        test(`${roleDef.name} - 待分拣订单列表`, () => {
          const pendingOrders = [
            { _id: 'order-001', orderNo: '丰淮商贸-20260811-0001', status: 'submitted' },
            { _id: 'order-002', orderNo: '丰淮商贸-20260811-0002', status: 'submitted' }
          ]
