...
const DEFAULT_PRODUCTS = [
  ...
  {material_code:'61', name:"75 黄色 120 包抽纸", spec:'1×1', pricing_mode:'case', unit_piece_qty:1, price_piece:70, price_unit:null, unit:'件', pinyin:'', is_adjustable:false},
...
Page({
  ...
  goEdit(e) {
    ...
    const statusIndex = item.pricing_mode === 'case' ? 1 : 0
    ...
    this.setData({
      ...
      formData: {
        ...
        stock: item.unit_piece_qty ? String(item.unit_piece_qty) : '',
        status: item.pricing_mode === 'case' ? 1 : 0
      },
      unitIndex,
      statusIndex
    })
  ...
  async saveProduct() {
    ...
    if (!formData.pricePiece) {
      wx.showToast({ title: '请输入件价', icon: 'none' })
      return
    }

    // 处理新字段
    const pricingMode = formData.status === 1 ? 'case' : 'piece'
    ...
    const productData = {
      ...
      pinyin: '',
      unit: formData.unit,
      price_piece: parseFloat(formData.pricePiece) || 0,
      price_unit: parseFloat(formData.priceUnit) || 0,
      unit_piece_qty: parseInt(formData.stock) || 1,
  ...
  async confirmImport() {
    const { defaultProducts, importOverride } = this.data
