...
exports.main = async (event, context) => {
  ...
  try {
    ...
    if (customers.data.length > 0) {
      ...
      passed++
      results.push({ test: '查询客户列表', status: 'pass', message: `获取到 ${customers.data.length} 个客户` })
    } else {
      console.log('❌ 失败：客户列表为空')
      failed++
      results.push({ test: '查询客户列表', status: 'fail', message: '客户列表为空' })
    }
  } catch (e) {
    console.log('❌ 失败:', e.message)
    failed++
    results.push({ test: '查询客户列表', status: 'fail', message: e.message })
  }
  ...
  try {
    ...
    if (products.data.length > 0) {
      ...
      failed++
      results.push({ test: '查询商品列表', status: 'fail', message: '商品列表为空' })
    }
  } catch (e) {
    console.log('❌ 失败:', e.message)
    failed++
    results.push({ test: '查询商品列表', status: 'fail', message: e.message })
  }
  ...
  try {
    ...
    if (!hasIssue) {
      ...
      passed++
      results.push({ test: '商品字段完整性', status: 'pass', message: '所有字段完整' })
    } else {
      console.log('❌ 失败:', issueMsg)
      failed++
      results.push({ test: '商品字段完整性', status: 'fail', message: issueMsg })
    }
  } catch (e) {
    console.log('❌ 失败:', e.message)
    failed++
    results.push({ test: '商品字段完整性', status: 'fail', message: e.message })
  }
  ...
  try {
    ...
    if (customerResult.data.length === 0) {
      ...
      failed++
      results.push({ test: '创建测试订单', status: 'fail', message: '没有可用客户' })
    } else {
      ...
      if (productResult.data.length === 0) {
        ...
        failed++
        results.push({ test: '创建测试订单', status: 'fail', message: '没有按件计价的商品' })
      } else {
    ...
    console.log(e.stack)
    failed++
    results.push({ test: '创建测试订单', status: 'fail', message: e.message })
  }
  ...
  try {
    ...
    if (orders.data.length > 0) {
      ...
      passed++
      results.push({ test: '查询订单列表', status: 'pass', message: `获取到 ${orders.data.length} 个订单` })
    } else {
      console.log('❌ 失败：订单列表为空')
      failed++
      results.push({ test: '查询订单列表', status: 'fail', message: '订单列表为空' })
    }
  } catch (e) {
    console.log('❌ 失败:', e.message)
    failed++
    results.push({ test: '查询订单列表', status: 'fail', message: e.message })
  }
  ...
  try {
    if (global.testOrderId) {
      ...
      if (order.data) {
        ...
        passed++
        results.push({ test: '查询订单详情', status: 'pass', message: '成功获取订单详情' })
      } else {
        console.log('❌ 失败：订单不存在')
        failed++
        results.push({ test: '查询订单详情', status: 'fail', message: '订单不存在' })
      }
    } else {
      console.log('⚠️  跳过：没有测试订单 ID')
      results.push({ test: '查询订单详情', status: 'skip', message: '没有测试订单 ID' })
    }
  } catch (e) {
    console.log('❌ 失败:', e.message)
    failed++
    results.push({ test: '查询订单详情', status: 'fail', message: e.message })
  }
  ...
  try {
    ...
    if (userResult.data.length > 0) {
      ...
      passed++
      results.push({ test: '权限校验', status: 'pass', message: '管理员用户存在' })
    } else {
      console.log('⚠️  警告：没有管理员用户，首次使用会自动创建')
      passed++
      results.push({ test: '权限校验', status: 'pass', message: '首次使用会自动创建管理员' })
    }
  } catch (e) {
    console.log('❌ 失败:', e.message)
    failed++
    results.push({ test: '权限校验', status: 'fail', message: e.message })
  }
  ...
  return {
    code: failed === 0 ? 0 : 1,
    message: failed === 0 ? '所有测试通过' : '部分测试失败',
    summary: {
      passed,
      failed,
      total: passed + failed,
