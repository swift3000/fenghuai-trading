...
# 说明：QA 钩子开/关各需重新部署 8 个云函数（约 6-8 分钟），全程约 20-25 分钟。
...
echo "==> [1/9] 权限逻辑单测（perm-logic）"
node tests/perm-logic-test.js

echo "==> [2/9] 权限 UI 测试（wx-perm-ui，需微信开发者工具模拟器）"
run_ui tests/wx-perm-ui-test.js

echo "==> [3/9] 页面走查（wx-pagewalk）"
run_ui tests/wx-pagewalk-test.js

echo "==> [4/9] 多层级深度巡检（wx-deepwalk：L2 页内 tab + L3 弹窗）"
run_ui tests/wx-deepwalk-test.js

echo "==> [5/9] 开启 QA 身份钩子（部署 8 函数 QA_IMPERSONATE=1，约 3-4 分钟）"
node tests/qa-toggle.js on

echo "==> [6/9] 多角色云端 403 拦截 + 开关即时生效（wx-role-sim）"
node tests/wx-role-sim-test.js

echo "==> [7/9] 全业务流程 E2E（wx-e2e-flow）"
node tests/wx-e2e-flow-test.js

echo "==> [8/9] 会员/权限管理（wx-member-mgmt）"
node tests/wx-member-mgmt-test.js

echo "==> [9/9] 关闭 QA 身份钩子（恢复生产安全态，约 3-4 分钟）"
