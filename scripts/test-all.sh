...
echo "==> [3/15] 页面走查（wx-pagewalk）"
run_ui tests/wx-pagewalk-test.js

echo "==> [4/15] 多层级深度巡检（wx-deepwalk：L2 页内 tab + L3 弹窗）"
run_ui tests/wx-deepwalk-test.js

echo "==> [5/15] 开启 QA 身份钩子（部署 8 函数 QA_IMPERSONATE=1，约 3-4 分钟）"
node tests/qa-toggle.js on

echo "==> [6/15] 多角色云端 403 拦截 + 开关即时生效（wx-role-sim）"
run_ui tests/wx-role-sim-test.js

echo "==> [7/15] 状态机终态守卫回归（order-state-guard，T53：已取消不可分拣/出库 + 重复出库幂等不覆盖）"
node tests/order-state-guard-test.js

echo "==> [8/15] CSV 公式注入回归（csv-injection，T59-R10：=/+ 开头单元格导出必被转义 + 导出权限 403）"
node tests/csv-injection-test.js

echo "==> [9/15] 深链路交叉回归（deep-chain-cross，T59-R14：多客户多订单 报表/台账/收款方式 三口径交叉守恒）"
node tests/deep-chain-cross-test.js

echo "==> [10/15] 性能基线终验（perf-baseline：10并发×核心5接口 P95<2s + 守恒）"
node tests/perf-baseline-test.js

echo "==> [11/15] 全业务流程 E2E（wx-e2e-flow）"
run_ui tests/wx-e2e-flow-test.js

echo "==> [12/15] 会员/权限管理（wx-member-mgmt）"
run_ui tests/wx-member-mgmt-test.js

echo "==> [13/15] 幂等专项验收（wx-empirical-idem：双击不重复登记/重复确认不重复入账）"
run_ui tests/wx-empirical-idem-test.js

echo "==> [14/15] 关闭 QA 身份钩子（恢复生产安全态，约 3-4 分钟）"
node tests/qa-toggle.js off

echo "==> [15/15] 收尾门禁：QA 残留校验（任一函数 QA_IMPERSONATE 非空或查询异常即失败）"
