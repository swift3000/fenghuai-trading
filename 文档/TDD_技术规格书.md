...
本项目为丰淮商贸的采购下单场景提供数字化解决方案，包含两个核心终端：

| 终端 | 用户角色 | 核心能力 |
|------|----------|----------|
| 微信小程序 | 采购员（一线） | 浏览商品、创建订单、客户区域选择、蓝牙打印小票 |
| Web 管理后台（极简静态页） | 管理员（内勤/老板） | 商品/客户管理、订单审核、Excel导出、打印 |
...
| 序号 | 费用项 | 金额（元） | 说明 |
|------|--------|-----------|------|
| 1 | 小程序端开发 | 1,800 | 微信原生 + Vant；商品浏览/下单/购物车/蓝牙打印 |
| 2 | Web 后台开发（单 HTML） | 1,200 | 纯 HTML+原生JS+SheetJS；订单/商品/客户/区域/用户 5 标签页 |
| 3 | 8 个云函数开发 | 1,000 | auth-login / data-query / product-write / customer-write / order-write / region-write / user-write / system-config |
...
- 合计：年度费用 **≈ 0 元**（超出免费额度部分约 10-50 元/月，按量付费）
