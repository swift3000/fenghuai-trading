...
**状态机（简化后：草稿 → 待分拣 → 已分拣 → 已出库 → 已完成）**：
```
draft ──提交──► submitted（待分拣）──一键分拣──► sorted（已分拣）──库管一步出库确认──► confirmed（已出库）──► completed（已完成）
  │                                                                                                                                       
  └───────────── cancelled ◄──────────────────────────────────────────────────────────────────────────────────────┘
               （提交后任意时刻可取消）

流转规则：
1. 下单员提交订单：draft → submitted（待分拣，写入 submitted_at）
...
5. 取消：提交后任意时刻可手动 cancelled（全员可取消，v4.3）
