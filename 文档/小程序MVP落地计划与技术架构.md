...
- 工作量基准：MVP 约 **30 人天**（1 人全栈 6–7 周；2 人并行 4 周）。

---

## 一、MVP 功能清单（最终范围，共 11 项）
...
| W2 核心业务 | 1.5 周 | 新建订单、分拣、库管、收款两步状态机、赊销页（客户台账/未结清/已结清 三栏） | 12 |
| W3 模板与适老 | 0.5 周 | 销售单打印/转发模板、全局字号缩放 | 4 |
| W4 联调上线 | 0.5 周 | 真机调试、蓝牙打印、体验版、审核发布 | 4 |
| **合计** | **约 3–3.5 周开发** | | **≈ 30 人天** |
...
/fenghuai-mini
  ...
  /cloudfunctions
    ...
    payment, print, export(3种Excel导出), report(预留)
  project.config.json
```
...
### 3.5 订单状态机（简化主线：草稿 → 待分拣 → 已分拣 → 已出库 → 已完成）
```
DRAFT(草稿) → SUBMITTED(待分拣)
...
状态转换在云函数 `order` 内集中校验；前端仅触发动作，不直接改状态。`CANCELED`(手动取消) 保留于主流程；`REJECTED`(已驳回·必填原因) 状态保留定义但当前 MVP 流程不触发（已取消，原属二期规划，不纳入主流程）。
